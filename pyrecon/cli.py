...
def main() -> None:
    ...
    # Parse open ports
    open_ports = parse_open_ports_from_gnmap(
        Path(str(base_prefix) + ".gnmap")
    )
    ...
    table.add_row(
        ", ".join(map(str, open_ports)) if open_ports else "None found"
    )
    ...
    if open_ports:
        ...
        console.print(
            "[yellow]No open ports parsed. Skipping thorough/vuln scans.[/yellow]"
        )
    ...
    if Confirm.ask("Run Feroxbuster?"):
        ...
        if not urls:
            console.print(
                "[yellow]No common web ports detected. Enter a URL manually.[/yellow]"
            )

            manual = Prompt.ask(
                "URL (e.g., http://target:8080)"
            ).strip()
        ...
        if urls:
            ...
            console.print(
                "[red]No URLs provided. Skipping Feroxbuster.[/red]"
            )
    ...
    console.print(
        f"[bold cyan]Saved to:[/bold cyan] {base_dir}"
    )
